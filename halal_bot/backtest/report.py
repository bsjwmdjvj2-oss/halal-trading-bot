...
def write_report(result: BacktestResult, universe: list[str], data_caveats: list[str],
    ...
    if benchmark is not None:
        benchmark.to_csv(run_dir / "spy_benchmark.csv", header=True)
    ...
    summary = {
        ...
        "spy_benchmark_ending_value": float(benchmark.iloc[-1]) if benchmark is not None else None,
    ...
    if benchmark is not None:
        benchmark.plot(ax=ax, label="S&P 500 (SPY)", linestyle="--")
        ax.legend()
    ax.set_title("Backtest Equity Curve vs. S&P 500")
...
def print_summary(result: BacktestResult, universe: list[str], data_caveats: list[str],
                   benchmark: pd.Series | None = None) -> None:
    m = result.metrics
    print("\n" + "=" * 60)
    ...
    if m:
        ...
        if benchmark is not None:
            spy_end = benchmark.iloc[-1]
            spy_return_pct = (spy_end / start_equity - 1) * 100
            strat_return_pct = (end_equity / start_equity - 1) * 100
            print(f"\nvs. S&P 500 (SPY), same starting capital{' + contribution schedule' if result.total_contributed else ''}:")
            print(f"  Strategy ending value: ${end_equity:,.2f}  ({strat_return_pct:+.2f}%)")
            print(f"  SPY ending value:      ${spy_end:,.2f}  ({spy_return_pct:+.2f}%)")
            print(f"  Strategy {'beat' if end_equity > spy_end else 'trailed'} SPY by ${abs(end_equity - spy_end):,.2f}")
    else:
